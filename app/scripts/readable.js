...
var jsdom = {
    env : function(options) {
        ...
        if(!html || !document) callback({error:'boo'});
        renderFrame = document.createElement("frame");
        renderFrame.setAttribute("style", "display:none");
        renderFrame.setAttribute("name", "_renderFrame_");
        renderFrame.setAttribute("id", "_renderFrame_");
        renderFrame.document.write(html);
        callback(null, renderFrame.document);
        return renderFrame.document;
    }
};
 ...
 * @return void
 **/
var prepDocument = function (document) {
 ...
 * @return Element
 **/
var grabArticle = function (document, preserveUnlikelyCandidates) {
    /**
    ...
    /**
     * Loop through all paragraphs, and assign a score
     * to them based on how content-y they look.
    ...
    candidates.forEach(function (candidate) {
         ...
         **/
        candidate.readability.contentScore = candidate.readability.contentScore
            * (1 - getLinkDensity(candidate));

        dbg('Candidate: ' + candidate + " (" + candidate.className
            + ":" + candidate.id + ") with score " + candidate.readability.contentScore);

        if (!topCandidate || candidate.readability.contentScore
    ...
    for (var i = 0, il = siblingNodes.length; i < il; i++) {
        ...
        dbg("Looking at sibling node: " + siblingNode + " ("
        ...
        dbg("Sibling has score " + (siblingNode.readability
            ? siblingNode.readability.contentScore : 'Unknown'));
        ...
        if (append) {
            ...
            /* Append sibling and subtract from our list because 
             it removes the node when you append to another node */
...
function cleanStyles (e) {
    ...
    if (typeof e.removeAttribute == 'function' && e.className
        != 'readability-styled') e.removeAttribute('style');
...
function cleanConditionally(e, tag) {
    ...
    for (var i = curTagsLength - 1; i >= 0; i--) {
        ...
        dbg("Cleaning Conditionally " + tagsList[i] + " (" + tagsList[i].className
            + ":" + tagsList[i].id + ")" + ((typeof tagsList[i].readability != 'undefined')
...
function read(html, options, callback) {
    ...
    if (html.indexOf('<') === -1) {
        $.get('http://127.0.0.1:8080/p/',
            { url : encodeURIComponent(html) },
            function( data ) {
                console.log(data);
                jsdomParse(null, null, data);
            });
    } else {
